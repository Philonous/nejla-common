...
getConfGenericMaybe ::
    ...
    (String -> Maybe a) ->
    -- | Environment variable to read option from
    String ->
    -- | Config name to read option from
    Conf.Name ->
    -- | Config object to read option from
    Conf.Config ->
    m (Maybe a)
...
getConfGeneric ::
    ...
    -- | Config name to read option from
    Conf.Name ->
    -- | Default value to use when option could not
...
getConfGeneric fromString env confName mbDefault conf = do
    ...
    case mbC of
        Nothing -> case mbDefault of
            ...
            Left e -> do
                $logError $
                    "Configuration of `" <> e <> "` is required. \n"
...
-}
getConf' ::
    (Conf.Configured a, MonadLogger m, MonadIO m, Read a) =>
    -- | Environment variable to read option from
    String ->
    -- | Config name to read option from
    Conf.Name ->
    -- | Default value to use when option could not be
