...
data LogEvent = LogEvent
    { -- | What sort of event this is (e.g. "login
      -- failed").
      logEventType :: !Text
    , -- | Log level
      logEventLevel :: !LogLevel
    , -- | Component that is logging the
...
-}
logEvent ::
    (MonadIO m, MonadLogger m, IsLogEvent a) =>
    a ->
    m ()
...
withFileLogger ::
    ToLogStr log =>
    -- | Base name of the log files, a timestamp is
    -- prepended to the base name
    FilePath ->
    -- | Timestamp format (e.g. "%F" for date). Same
