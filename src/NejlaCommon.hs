        ...
        case info of
            TyConI (DataD [] name [] _ [RecC cName cFields] _) -> do
                ...
                case compare (length rf) (length removedFields) of
                    ...
                    GT ->
                        reportWarning $
                            "Could not find all fields to remove for "
                                <> show name
                                <> " ("
                ...
                case compare (length mf) (length maybeFields) of
                    ...
                    GT ->
                        reportWarning $
                            "Could not find all fields to make optional for "
                                <> show name
                                <> " ("
...
{- | Extends a given type with an extra field and derives 'FromJSON', 'ToJSON'
 ...
 @
-}
data WithField (name :: Symbol) fieldType baseType = WithField
...
instance
    (KnownSymbol name, ToJSON fieldType, ToJSON baseType) =>
    ToJSON (WithField name fieldType baseType)
    where
    toJSON wf =
...
instance
    (KnownSymbol name, FromJSON fieldType, FromJSON baseType) =>
    FromJSON (WithField name fieldType baseType)
    where
